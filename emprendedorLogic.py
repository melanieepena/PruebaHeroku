...
class emprendedorLogic(Logic):
    ...
    def insertNewEmprendedor(
        ...
        sql = (
            "insert into fishingdb1.emprendedor (id, nombre, email, telefono, id_usuario, pais, ciudad, biografia) "
    ...
    def getNewEmprendedor(self, name, email, phone, id_user, country, city, biografia):
        dataBase = self.get_databaseXObj()
        sql = "select * from fishingdb1.emprendedor " + f"where id_usuario = {id_user};"
    ...
    def getAllEmprendedores(self):
        dataBase = self.get_databaseXObj()
        sql = "select * from fishingdb1.emprendedor;"
        data = dataBase.executeQuery(sql)
        data = self.tupleToDictionaryList(data, self.keys)
        return data
    ...
    def deleteEmprendedor(self, id):
        database = self.get_databaseXObj()
        sql = f"delete from fishingdb1.emprendedor where emprendedor.id = '{id}';"
        rows = database.executeNonQueryRows(sql)
        return rows
