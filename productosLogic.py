...
class productosLogic(Logic):
    ...
    def insertNewProducto(self, name, picture, desc, costo, precio, patente, id_emp):
        ...
        sql = (
            "insert into fishingdb1.productos (id, nombre, foto, descripcion, costo_unitario, precio_venta, patente, id_emprendimiento) "
    ...
    def deleteProducto(self, id_prod):
        database = self.get_databaseXObj()
        sql = "delete from fishingdb1.productos " + f"where id = {id_prod};"
        rows = database.executeNonQueryRows(sql)
        return rows
    ...
    def getNewProducto(self, id_prod):
        dataBase = self.get_databaseXObj()
        sql = "select * from fishingdb1.productos " + f"where id = {id_prod};"
    ...
    def getAllProductoLen(self):
        dataBase = self.get_databaseXObj()
        sql = "select * from fishingdb1.productos "
        data = dataBase.executeQuery(sql)
        return data
