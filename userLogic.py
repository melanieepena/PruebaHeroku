...
class UserLogic(Logic):
    ...
    def getUser(self, user, password):
        ...
        sql = (
            "SELECT * FROM fishingdb1.usuario "
    ...
    def insertNewUser(self, user, password, rol):
        database = self.get_databaseXObj()
        sql = (
            "insert into fishingdb1.usuario (id, usuario, password, rol) "
    ...
    def checkUserInUsuario(self, user, rol):
        dataBase = self.get_databaseXObj()
        sql = (
            "SELECT usuario.usuario FROM fishingdb1.usuario "
    ...
    def getUserByUser(self, user):
        dataBase = self.get_databaseXObj()
        sql = "SELECT * FROM fishingdb1.usuario " + f"where usuario.usuario = '{user}';"
        print(sql)
        data = dataBase.executeQuery(sql)
        data = self.tupleToDictionaryList(data, self.keys)
        if len(data) > 0:
