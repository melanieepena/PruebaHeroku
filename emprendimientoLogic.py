...
class emprendimientoLogic(Logic):
    ...
    def insertNewEmprendimiento(
        ...
        sql = (
            "insert into fishingdb1.emprendimiento (id, estado, descripcion, historia, eslogan, inversion_inicial, fecha_fundacion, venta_año_anterior, "
    ...
    def getEmprendimientoByName(self, nombre):
        dataBase = self.get_databaseXObj()
        sql = (
            "SELECT * FROM fishingdb1.emprendimiento "
    ...
    def checkEmprendimiento(self, name):
        dataBase = self.get_databaseXObj()
        sql = (
            "SELECT emprendimiento.nombre FROM fishingdb1.emprendimiento "
    ...
    def deleteEmprendimiento(self, id):
        database = self.get_databaseXObj()
        sql = f"delete from fishingdb1.emprendimiento where emprendimiento.id = '{id}';"
        rows = database.executeNonQueryRows(sql)
        return rows

    def getAllEmprendimientoLen(self):
        dataBase = self.get_databaseXObj()
        sql = "SELECT * FROM fishingdb1.emprendimiento;"
        data = dataBase.executeQuery(sql)
        data = self.tupleToDictionaryList(data, self.keys)
        return data
